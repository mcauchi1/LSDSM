...
if allow_pat_plots
    ...
    for i=1:no_of_plots
        figure;
        scatter(t_all, squeeze(data_observed(i).y(biomarker_to_plot,:)), 'LineWidth', 2);
        xlabel('Time');
        legend('y');
...
est_rmse_states = LSDSM_ALLFUNCS.find_rmse_states(data_latent, data_observed, RTS_traj.mu_tilde);
...
% Initialise arrays for time-dependent Brier score, prediction error, and
...
for l_var=1:length(landmark_t_arr)
    ...
    LSDSM_ALLFUNCS.plot_bs_pe_auc(pat_data_reduced, bs_test, pe_test, auc_test, landmark_t, censor_time, t_multiplier, Delta)

    if allow_pat_plots
        for i=1:no_of_plots
            LSDSM_ALLFUNCS.plot_forecast_surv(pat_data_reduced(i), landmark_t, t_all, Delta)
        end
    end
end
