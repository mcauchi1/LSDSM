...
horiz_to_test <- c(1,2,5) # horizons to test in years
no_of_folds <- 5 # number of folds for x-fold cross-validation
...
set.seed(1)
rand_seq <- sample(seq_len(nrow(orig_df.id)), size = nrow(orig_df.id))
fold_size_frac <- 1 / no_of_folds
no_of_pats <- nrow(orig_df.id)

# create a list of dataframes to store the results
perf_metrics_df <- list()
for (t_horiz in horiz_to_test) {
  perf_metrics_df[[t_horiz]] <- data.frame(matrix(vector(), 0, 3,
                                                dimnames=list(c(), c("bs", "pe", "auc"))),
                                         stringsAsFactors=F)
}


for (k in 1:no_of_folds) { # for every fold
  # indices left out (of training) in this k-th iteration of cross-validation
  test_idx <- rand_seq[floor((k-1)*fold_size_frac*no_of_pats+1):floor(k*fold_size_frac*no_of_pats)]
  
  # create training and testing data sets
  train_data.id <- orig_df.id[-test_idx, ]
  test_data.id <- orig_df.id[test_idx, ]
  ...
  train_file_loc = paste(parent_folder_data, "PBC_dataset_train_fold", k, ".csv", sep="")
  test_file_loc = paste(parent_folder_data, "PBC_dataset_test_fold", k, ".csv", sep="")
  ...
  for (landmark in landmarks_to_test) {
    ...
    for (t_horiz in horiz_to_test) { # for every horizon to consider
      ...
      # fold 2, landmark 1
      # ...
      perf_metrics_df[[t_horiz]][nrow(perf_metrics_df[[t_horiz]])+1, ] = perf_metrics_vec
    }
  }
}

# store the results for all horizons
if (rand_int_only) {
end_string <- sprintf("several_landmarks_horiz_%.1f_fold_%d_bs_pe_auc_rand_int_only", 
                      t_horiz, no_of_folds)
} else {
  end_string <- sprintf("several_landmarks_horiz_%.1f_fold_%d_bs_pe_auc", 
                        t_horiz, no_of_folds)
}
...
for (t_horiz in horiz_to_test) {
  ...
  write.csv(perf_metrics_df[[t_horiz]], results_file_loc, row.names = FALSE)
}
